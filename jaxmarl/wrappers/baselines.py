...
class MPELogEnvState:
    ...
    episode_returns: float
    episode_lengths: int
    environment_specific: int
    returned_episode_returns: float
    returned_episode_lengths: int
    returned_environment_specific: int
...
class MPELogWrapper(LogWrapper):
    ...
    def reset(self, key: chex.PRNGKey) -> Tuple[chex.Array, State]:
        ...
        state = MPELogEnvState(
            ...
            jnp.zeros((self._env.num_agents,)),
            jnp.zeros((self._env.num_agents,)),
            self._env.environment_specific_reset,
            jnp.zeros((self._env.num_agents,)),
            jnp.zeros((self._env.num_agents,)),
            self._env.environment_specific_reset,
        )
    ...
    def step(
        ...
        new_environment_specific = {}
        if "environment_specific" in info:
            new_environment_specific = jax.tree_util.tree_map(
                lambda x, y: x + y,
                info["environment_specific"],
                state.environment_specific,
            )
        ...
        state = MPELogEnvState(
            ...
            episode_returns=new_episode_return * (1 - ep_done),
            episode_lengths=new_episode_length * (1 - ep_done),
            environment_specific=jax.tree_util.tree_map(
                lambda x: x * (1 - ep_done), new_environment_specific
            ),
            returned_episode_returns=state.returned_episode_returns * (1 - ep_done)
            + new_episode_return * ep_done,
            returned_episode_lengths=state.returned_episode_lengths * (1 - ep_done)
            + new_episode_length * ep_done,
            returned_environment_specific=jax.tree_util.tree_map(
                lambda x, y: x * (1 - ep_done) + y * ep_done,
                state.environment_specific,
                new_environment_specific,
            ),
        )
        ...
        info["returned_episode_lengths"] = state.returned_episode_lengths
        info["returned_episode"] = jnp.full((self._env.num_agents,), ep_done)
        info["environment_specific"] = state.returned_environment_specific
        return obs, state, reward, done, info
